import { IRequestTerminationConsultancy, IRequestTerminationConsultancyRequestData, IRequestTerminationConsultancySearchFields } from "@/features/asesores/types/solicitud-cese-asesoria"
import { IListAvailableAdvisorList, ISearchCriteriaAvailableAdvisorList, IStudentCessationRequest } from "../types/solicitud-cese-asesoria";


const mockSolicitudesCeseAsesoria: IRequestTerminationConsultancy = 
    {
        "requestTermmination": [
            {
                "id": 1,
                "registerTime": new Date("2025-04-16T10:30:00Z"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date(),
                "assessor": {
                    "id": 1,
                    "name": "Josue",
                    "lastName": "Salinas",
                    "email" : "josalin@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "https://i.pravatar.cc/150?u=P021",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 4,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 5,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 6,
                        "name": "Martin",
                        "lastName": "Solis",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 2,
                "registerTime": new Date("2024-05-12"),
                "status": "approved",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date(),
                "assessor": {
                    "id": 1,
                    "name": "Jose",
                    "lastName": "Cerna",
                    "email" : "joserna@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "https://i.pravatar.cc/150?u=P021",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Elio",
                        "lastName": "Lopez",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 4,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 5,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 6,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 7,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 8,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 9,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 10,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }

                ]
            },
            {
                "id": 3,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Fernando",
                    "lastName": "Salas",
                    "email" : "fersal@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 4,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Rocio",
                    "lastName": "Salinas",
                    "email" : "joserna@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 5,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date(),
                "assessor": {
                    "id": 1,
                    "name": "Francisco",
                    "lastName": "Rojas",
                    "email" : "franro@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 6,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Claudio",
                    "lastName": "Quintero",
                    "email" : "clauquin@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 7,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Daniel",
                    "lastName": "Palermo",
                    "email" : "danpa@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 8,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Zoila",
                    "lastName": "Cisneros",
                    "email" : "zoicis@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 9,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Juan",
                    "lastName": "De La Cruz",
                    "email" : "juanDel@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 10,
                "registerTime": new Date("2024-05-12"),
                "status": "approved",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Maria",
                    "lastName": "Nuñez",
                    "email" : "marnuñ@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 11,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 12,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 13,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 14,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 15,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 16,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 17,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 18,
                "registerTime": new Date("2024-05-12"),
                "status": "rejected",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 19,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 20,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 21,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 22,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 23,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 24,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 25,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 26,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 27,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 28,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 29,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 30,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 31,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 32,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 33,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 34,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 35,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 36,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 37,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 38,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 39,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 40,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 41,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 42,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 43,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 44,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 45,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 46,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 47,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 48,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 49,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 50,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 51,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 52,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 53,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 54,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 55,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 56,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 57,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 58,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 59,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 60,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 61,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 62,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 63,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
            {
                "id": 64,
                "registerTime": new Date("2024-05-12"),
                "status": "pending",
                "reason": "no time",
                "response": "Could not proceed",
                "responseTime": new Date("2024-05-12"),
                "assessor": {
                    "id": 1,
                    "name": "Mario",
                    "lastName": "Contreras",
                    "email" : "martreras@email.com",
                    "quantityCurrentProyects": 10,
                    "urlPhoto": "",
                },
                "students": [
                    {
                        "id": 1,
                        "name": "Mario",
                        "lastName": "Cartagena",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 2,
                        "name": "Pedro",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    },
                    {
                        "id": 3,
                        "name": "Adrian",
                        "lastName": "Salinas",
                        "topic": {
                            "name": "Inteligencia Artificial"
                        }
                    }
                ]
            },
         
            
        ],
        "totalPages": 1
    }
                
const mockedStudentList = [
    {
        id: 1,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    },
    {
        id: 2,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    },
    {
        id: 3,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    },
    {
        id: 4,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    },
    {
        id: 5,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    },
    {
        id: 6,
        firstName: "Facundo",
        lastName: "Fernandez",
        email: "a20151515@email.com",
        code: "20151515",
        thematicArea: {
            id: 1,
            description: "Software development"
        },
        profilePicture: "https://i.pravatar.cc/150?u=P021",
        advisorId: null,
    }

]
    

const mockedAdvisorList = {
    "assessors": [
        {
      id: 1,
      firstName: "Saul",
      lastName: "Rojas",
      email: "saulro@email.com",
      code: "40402121",
      thematicAreas: [
        {
            id: 1,
            description: "Software development"
        },
      ],
      assignedStudentsQuantity: 1,
      capacity: 3,
      profilePicture: "https://i.pravatar.cc/150?u=P021",
    },
    {
        id: 2,
        firstName: "Alex",
        lastName: "Roman",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 3,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 4,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 5,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 6,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 7,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 8,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 9,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 10,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 11,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 12,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
      {
        id: 13,
        firstName: "Mariano",
        lastName: "Cerna",
        email: "saulro@email.com",
        code: "40402121",
        thematicAreas: [
          {
              id: 1,
              description: "Software development"
          },
        ],
        assignedStudentsQuantity: 1,
        capacity: 3,
        profilePicture: "https://i.pravatar.cc/150?u=P021",
      },
    ],
    totalPages: 10
}

    
                
// Service to get all request for consultancy termination
export async function getTerminationConsultancyList(
    searchCriteria: IRequestTerminationConsultancySearchFields
): Promise<IRequestTerminationConsultancy> {
    const ELEMENTS_PER_PAGE = 10
    //const totalPages = mockSolicitudesCeseAsesoria.totalPages
    const terminationRequests = mockSolicitudesCeseAsesoria.requestTermmination
    const filteredTerminationRequestsInput = terminationRequests.filter(terminationRequest => {
        const fullName = `${terminationRequest.assessor.name} ${terminationRequest.assessor.lastName}`;
        return fullName.toLowerCase().includes(searchCriteria.fullNameEmail.toLowerCase()) || terminationRequest.assessor.email.toLowerCase().includes(searchCriteria.fullNameEmail.toLowerCase());
    });
    const filteredTerminationRequestsStatus = filteredTerminationRequestsInput.filter(terminationRequest=>{
        return  (searchCriteria.status === "answered" ? (terminationRequest.status === "approved" || terminationRequest.status === "rejected") : (terminationRequest.status === searchCriteria.status))
    })
    const filteredTerminationRequestsPagination = filteredTerminationRequestsStatus.slice(ELEMENTS_PER_PAGE * (searchCriteria.page - 1) , ELEMENTS_PER_PAGE * searchCriteria.page)
    if (filteredTerminationRequestsPagination.length > 0)
        filteredTerminationRequestsPagination[0].responseTime = new Date()

    await new Promise(resolve => setTimeout(resolve, 500));

    return {
        "requestTermmination": 	filteredTerminationRequestsPagination,
	    "totalPages": Math.ceil(filteredTerminationRequestsStatus.length / ELEMENTS_PER_PAGE)
    }
}
  


// Service to get all request for consultancy termination
export async function getTerminationConsultancyRequest(
    idRequest: number | null
): Promise<IRequestTerminationConsultancyRequestData | null> {
    if (idRequest === null)
        return null
    
    const terminationRequests = mockSolicitudesCeseAsesoria.requestTermmination
    const selectedTerminationRequest = terminationRequests.filter(terminationRequest=>{
        return  terminationRequest.id === idRequest
    })
    await new Promise(resolve => setTimeout(resolve, 500));
    return selectedTerminationRequest[0] ?? null
    
}


// Service to get all student list information for consultancy termination
export async function getTerminationRequestStudentList(
    idRequest: number | null
): Promise<Array<IStudentCessationRequest> | null> {
    if (idRequest === null)
        return null
    const studentList = mockedStudentList;
    
    await new Promise(resolve => setTimeout(resolve, 500));
    return studentList;
    
}




// Service to get all assessor list information for consultancy termination
export async function getTerminationRequestAssessorList(
    searchCriteriaAvailableAdvisorList: ISearchCriteriaAvailableAdvisorList
): Promise<IListAvailableAdvisorList | null> {
    const ELEMENTS_PER_PAGE = 10;
    if (searchCriteriaAvailableAdvisorList.idThematicArea === null)
        return null
    const initialAdvisors = mockedAdvisorList.assessors.filter((advisor)=>advisor.thematicAreas?.some((thematicArea)=>thematicArea.id === searchCriteriaAvailableAdvisorList.idThematicArea))
    const filteredTerminationRequestsInput = initialAdvisors.filter(advisor => {
        const fullName = `${advisor.firstName} ${advisor.lastName}`;
        return fullName.toLowerCase().includes(searchCriteriaAvailableAdvisorList.fullNameEmailCode.toLowerCase()) || advisor.email.toLowerCase().includes(searchCriteriaAvailableAdvisorList.fullNameEmailCode.toLowerCase()) || advisor.code.toLowerCase().includes(searchCriteriaAvailableAdvisorList.fullNameEmailCode.toLowerCase());
    });

    const filteredTerminationRequestsPagination = filteredTerminationRequestsInput.slice(ELEMENTS_PER_PAGE * (searchCriteriaAvailableAdvisorList.page - 1) , ELEMENTS_PER_PAGE * searchCriteriaAvailableAdvisorList.page)
    await new Promise(resolve => setTimeout(resolve, 2500));
    return {
        "advisors": filteredTerminationRequestsPagination,
	    "totalPages": Math.ceil(filteredTerminationRequestsInput.length / ELEMENTS_PER_PAGE)
    }
    
}

