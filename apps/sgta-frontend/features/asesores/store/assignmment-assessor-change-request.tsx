...
import {
  IAssessorChangeRequestAdvisorAssign,
  IAssessorChangeRequestStudentAssign,
...
interface IAssessorChangeRequestAssignmentState {
  ...
  getAssignedAdvisor: (
    studentId: number,
  ) => IAssessorChangeRequestAdvisorAssign | null;
...
export const useAssessorChangeAssignmentStore =
  create<IAssessorChangeRequestAssignmentState>((set, get) => ({
    students: [],
    advisors: [],
      ...
      if (!exists) {
        set({
          assignedAdvisors: [
            ...
            {
              ...advisor,
              assignedStudentsQuantity: advisor.assignedStudentsQuantity + 1,
            },
          ],
        });
      } else {
        const updatedAdvisors = assignedAdvisors.map((a) =>
          a.id === advisor.id
            ? { ...a, assignedStudentsQuantity: a.assignedStudentsQuantity + 1 }
            : a,
        );
    ...
    removeAssignedAdvisor: (advisorId) => {
      const { assignedAdvisors } = get();
      const { students } = get();
      const quantityOfAdvisorReferencesStudents = students.filter(
        (student) => student.advisorId === advisorId,
      ).length;
    ...
    unassignAdvisor: (studentId) => {
      ...
      const { students } = get();
      const updatedStudents = students.map((student) =>
        student.id === studentId ? { ...student, advisorId: null } : student,
      );
      const updatedSelectedStudent = updatedStudents.find(
        (student) => student.id === selectedStudent?.id,
      );
    ...
    getAdvisorAssignedCount: (advisorId) => {
      const { assignedStudents } = get();
      return Object.values(assignedStudents).filter((id) => id === advisorId)
        .length;
    },
