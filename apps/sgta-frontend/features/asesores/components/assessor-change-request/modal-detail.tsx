...
import {
  IAssessorChangeApproveModalProps,
...
const ApproveAssessorChangeModal: React.FC<
  IAssessorChangeApproveModalProps
> = ({ isOpen, onClose, idRequest }) => {
  const { isLoading: loadingRequestDetail, data: dataRequestDetail } =
    useRequestAssessorChangeDetail(idRequest);
  const statusConfig: Record<
    IRequestAssessorChangeRequestData["status"],
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="w-full sm:max-w-[800px]">
        ...
        {(() => {
            ...
            return (
              <Card className="mb-3 shadow-sm border">
                <CardHeader className="py-4 px-4 bg-gray-50 flex flex-row justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-medium px-2 py-0.5 rounded ${statusConfig[dataRequestDetail.status]?.color}`}
                  >
                ...
                <CardContent className="py-3 px-4 text-xs h-full">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {dataRequestDetail.previousAssessors.map((advisor) => (
                        <Card
                          key={advisor.id}
                          className="border shadow-none bg-muted/50"
                        >
                  ...
                  {dataRequestDetail.status === "rejected" &&
                    dataRequestDetail.response && (
          ...
          return (
            <div>
              No se encontró informacion sobre esta solicitud de cambio de
              asesor
            </div>
          );
        })()}
