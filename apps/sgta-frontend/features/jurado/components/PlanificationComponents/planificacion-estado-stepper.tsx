...
export default function PlanificacionEstadoStepper({ estadoPlan }: Props) {
  ...
  // Determinar si un paso está completado o es activo
        ...
        {steps.map((step, index) => (
          ...
          >
            <span className="min-w-7 min-h-7 group inline-flex items-center text-xs align-middle">
              ...
              `}
              >
                {/* Número del paso (visible cuando no está completado) */}
                <span className={isCompleted(step.id) ? "hidden" : ""}>
                  {index + 1}
                </span>

                {/* Icono de check (visible cuando está completado) */}
                {isCompleted(step.id) && (
                  <svg
            ...
            {/* Línea conectora entre pasos (oculta para el último paso) */}
