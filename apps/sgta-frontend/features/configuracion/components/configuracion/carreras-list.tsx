...
export function CarrerasList() {
  ...
  const carrerasFiltradas = carreras.filter(
    (carrera) =>
      carrera.facultad ===
      unidadesAcademicas.find((u) => u.id.toString() === unidadAcademica)
        ?.nombre,
  );
            ...
            {carrerasFiltradas.map((carrera) => (
              <tr key={carrera.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-sm">
                  <Badge
                    variant={
                      carrera.estado === "Activo" ? "default" : "secondary"
                    }
                  >
                    {carrera.estado}
                  </Badge>
                ...
                <td className="px-4 py-3 text-sm">
                  <div className="flex gap-2">
                    <Link
                      href={`/administrador/configuracion/carreras/${carrera.id}`}
                    >
