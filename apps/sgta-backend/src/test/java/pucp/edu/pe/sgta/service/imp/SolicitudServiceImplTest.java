...
public class SolicitudServiceImplTest {
	...
	void testFindAllSolicitudesCambioAsesor_ReturnsPaginatedResults() {
		...
		// Mocks
		when(solicitudRepository.findByTipoSolicitudNombre("Cambio Asesor")).thenReturn(List.of(solicitud));

		when(usuarioXSolicitudRepository.findBySolicitud(solicitud)).thenReturn(List.of(relAsesor, relEstudiante));

		when(subAreaConocimientoXTemaRepository.findFirstByTemaIdAndActivoTrue(1)).thenReturn(subAreaXTema);
	...
	void testRechazarSolicitudCambioAsesor_SuccessfulRejection() {
		...
		// Act
		RechazoSolicitudCambioAsesorResponseDto result = solicitudService.rechazarSolicitudCambioAsesor(solicitudId,
				responseText);
	...
	void testAprobarSolicitudCambioAsesor_SuccessfulApproval() {
		...
		// Act
		AprobarSolicitudCambioAsesorResponseDto result = solicitudService.aprobarSolicitudCambioAsesor(solicitudId,
				responseText);
