...
class TemaServiceImplTest {
    ...
    void listarTemasCicloActualXEtapaFormativa() {
        ...
        assertEquals("codigo1", result.get(0).getCodigo()); // Comprobamos el código del primer tema
        assertEquals("Titulo Tema 1", result.get(0).getTitulo()); // Comprobamos el título del primer tema

        assertEquals(2, result.get(1).getId()); // Comprobamos el ID del segundo tema
        assertEquals("codigo2", result.get(1).getCodigo()); // Comprobamos el código del segundo tema
