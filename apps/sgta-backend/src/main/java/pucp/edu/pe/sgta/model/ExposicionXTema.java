...
public class ExposicionXTema {
    ...
    @JoinColumn(
            name = "exposicion_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_ext_exposicion")
    )
    ...
    @JoinColumn(
            name = "tema_id",
            nullable = false,
            foreignKey = @ForeignKey(name = "fk_ext_tema")
    )
    private Tema tema;

    @ManyToOne(fetch = FetchType.LAZY)
    @JoinColumn(
            name = "bloque_horario_exposicion_id",
            foreignKey = @ForeignKey(name = "fk_ext_bloque_horario")
    )
    private BloqueHorarioExposicion bloqueHorarioExposicion;

    @Column(name = "revision_criterio_x_exposicion_id")
    private Integer revisionCriterioXExposicionId;
