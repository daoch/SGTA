...
public interface ExposicionRepository extends JpaRepository<Exposicion, Integer> {
	...
	@Query(value = "SELECT * FROM listar_exposiciones_x_etapa_formativa_x_ciclo(:etapaFormativaXCicloId)",
			nativeQuery = true)
	...
	@Query(value = """
			SELECT *
			  FROM listar_exposicion_x_ciclo_actual_etapa_formativa(
			    :etapa_formativa_id)
			""", nativeQuery = true)
	List<Object[]> listarExposicionXCicloActualEtapaFormativa(@Param("etapa_formativa_id") Integer etapaFormativaId);
	...
	@Query(value = "SELECT * FROM listar_exposiciones_sin_inicializar_cicloactual_por_etapa_formativa(:etapaFormativaId)",
			nativeQuery = true)
