...
public interface EtapaFormativaRepository extends JpaRepository<EtapaFormativa, Integer> {
	...
	@Query(value = "SELECT * FROM obtener_exposiciones_por_etapa_formativa_por_tema(:etapaFormativaId, :temaId)",
			nativeQuery = true)
	...
	@Modifying
	@Transactional
	@Query(value = "UPDATE etapa_formativa SET duracion_exposicion = CAST(:duracion AS interval) WHERE etapa_formativa_id = :id",
			nativeQuery = true)
