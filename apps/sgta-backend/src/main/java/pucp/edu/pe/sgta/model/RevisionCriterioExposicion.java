...
public class RevisionCriterioExposicion {
	...
	@JoinColumn(name = "exposicion_x_tema_id", nullable = false,
			foreignKey = @ForeignKey(name = "fk_rcxe_exposicion_x_tema"))
	private ExposicionXTema exposicionXTema;

	@ManyToOne(fetch = FetchType.LAZY, optional = false)
	@JoinColumn(name = "criterio_exposicion_id", nullable = false,
			foreignKey = @ForeignKey(name = "fk_rcxe_criterio_exposicion"))
